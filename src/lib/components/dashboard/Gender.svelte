...
// @ts-nocheck
    ...
    $: {
        ...
        paginatedItems = search
			? items.filter((gender) => {
					return (
						gender.gender.match(RegExp(search, 'gi')) ||
						gender.gender.match(RegExp(search, 'gi'))
					);
			  })
			: items;
...
<div class="h-full w-full">
                    ...
                    {#if paginatedItems}
                        {#each paginatedItems as item}
                            <tr class="border-b">
                                ...
                                <td class="w-min">
                                    <button class="text-center p-1 w-min my-2 bg-green-300 rounded-md" on:click={(e) => {showEditGenderModal = !showEditGenderModal; editId = item._id; editGender = item.gender; editDescription = item.description}}>Edit</button>
                                    <button class="text-center p-1 w-min my-2 bg-red-300 rounded-md" on:click={() => {deleteGender(item._id)}}>Delete</button>
                                </td    >
                            </tr>
                        {/each}
                    {/if}
    ...
    <EditGenderModal bind:showEditGenderModal bind:editId bind:editGender bind:editDescription {loadGenders}/>
