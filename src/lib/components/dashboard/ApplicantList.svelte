...
<div class="h-full w-full">
							...
							<tr class="border-2 border-black mx-5x">
								...
								<td>
									{#if item.pending === 'Pending'}
										<span
											class="bg-yellow-500 text-black text-xl font-medium mr-2 px-2 py-2 rounded"
											>Pending</span
										>
									{:else if item.pending === 'Approve'}
										<span class="bg-green-500 text-white text-xl font-medium mr-2 px-2 py-2 rounded"
											>Approved</span
										>
									{:else if item.pending === 'Declined'}
										<span class="bg-red-500 text-white text-xl font-medium mr-2 px-2 py-2 rounded"
											>Declined</span
										>
									{/if}
								</td>
								<td class="w-min">
									<button
										class="text-white bg-green-500 hover:bg-green-500 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2"
										on:click={() => {
											showEditApplicantModal = !showEditApplicantModal;
											editId = item._id;
											editMartialStatus = item.martial_status;
											editFamilyRole = item.family_role;
											editPending = item.pending;
									...
									>
									<button
										class="text-white bg-yellow-500 hover:bg-yellow-500 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2"
										on:click={clickHandler}>Show</button
									>
									<button
										class="text-white bg-red-700 hover:bg-red-800 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2"
										on:click={() => {
											deleteApplicant(item._id);
										}}>Delete</button
									>
								</td>
							</tr>
	...
	<EditApplicantList
		bind:showEditApplicantModal
		bind:editId
		bind:editMartialStatus
		bind:editFamilyRole
		bind:editPending
		bind:completeName
		bind:email
