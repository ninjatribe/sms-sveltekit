...
<div class="fixed z-10 inset-0 overflow-y-auto {showEditApplicantModal ? 'block' : 'hidden'}">
	<div
		class="bg-white overflow-hidden transform transition-all max-w-4xl mx-auto my-20"
		role="dialog"
		aria-modal="true"
		aria-labelledby="modal-headline"
	>
		<div class="p-6 bg-blue-900 text-white ">
			<h2 class="text-lg font-medium leading-6" id="modal-headline">&nbps;</h2>
		</div>

		<div class="mt-4">
			<div class="grid grid-cols-2 gap-2">
				<div class="mb-4">
					<label for="firstName" class="block mb-2 font-bold text-blue-900">Name:</label>
					<input
			...
			<div class="grid grid-cols-2 gap-2">
				<div class="mb-4">
					<label for="Martial_Status" class="block mb-2 font-bold text-blue-900"
						>Martial Status:</label
					>
					<input
						bind:value={editMartialStatus}
						class="w-full px-4 py-2 text-gray-700 bg-white border rounded appearance-none focus:outline-none focus:shadow-outline class:active:bg-gray-100 class:focus:outline-none class:focus:bg-gray-100"
					/>
				</div>
				<div class="mb-4">
			...
			<div class="p-5">
				<button
					type="button"
					class="px-4 py-2 font-bold text-white bg-red-900 rounded float-left"
					on:click={handleClose}>Close</button
				>
				<button
					type="submit"
					class="px-4 py-2 font-bold text-white bg-blue-900 rounded float-right"
					on:click={() => {
						updateApplicant();
					}}>Save</button
				>
