<script>
    ...
    <div class="w-full h-full flex justify-center items-center absolute top-0 z-50 {showEditStudentModal ? "block" : "hidden"}">
        ...
        <div class="flex flex-col justify-around items-center w-72 h-100 bg-slate-300">
            ...
            <div>
                <button class="bg-blue-300 w-14 p-2 border-2 rounded-lg" on:click={() => {updateStudent()}}>Save</button>
                <button class="bg-red-300 w-14 p-2 border-2 rounded-lg" on:click={() => {showEditStudentModal = !showEditStudentModal}}>Close</button>
            </div>
        </div>
    </div>