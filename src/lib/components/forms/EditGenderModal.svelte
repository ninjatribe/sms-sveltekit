...
<div class="w-full h-full flex justify-center items-center absolute top-0 z-50 {showEditGenderModal ? "block" : "hidden"}">
    <!-- "Main" div -->
    <div class="flex flex-col justify-around items-center w-72 h-60 bg-slate-300">
        ...
        <div>
            <button class="bg-blue-300 w-14 p-2 border-2 rounded-lg" on:click={() => {updateGender()}}>Save</button>
            <button class="bg-red-300 w-14 p-2 border-2 rounded-lg" on:click={() => {showEditGenderModal = !showEditGenderModal}}>Close</button>
        </div>
    </div>
</div>